...
class MemoryStructurePools {
  ...
  MemoryStructurePool<T> register<T extends NativeType>(int size) {
    final pool = memory_pool_create(_memory, size).systemCheck();
  ...
  @inline
  int size<T extends Struct>() => _pools[T.hashCode]!.ref.object_size;
...
class MemoryStructurePool<T extends NativeType> {
  ...
  @inline
  int size() => _pool.ref.object_size;

  @inline
  Pointer<T> allocate() => memory_pool_allocate(_pool).systemCheck().cast();

...
static void* test_threading_run(void* thread)
{
    ...
    do
    {
        casted->test_mediator = test_mediator_initialize(false);
    } while (!casted->test_mediator || ((struct mediator_native*)casted->test_mediator)->descriptor <= 0);
    mediator_native_register_callback((struct mediator_native*)casted->test_mediator, 0, 0, test_threading_call_dart_callback);
    casted->alive = true;
    ...
    while (casted->alive)
    {
        mediator_native_process_timeout((struct mediator_native*)casted->test_mediator);
    }
    test_mediator_destroy((struct mediator_native*)casted->test_mediator, false);
...
bool test_threading_initialize(int32_t thread_count, int32_t isolates_count, int32_t per_thread_messages_count)
{
    ...
    for (int32_t thread_id = 0; thread_id < thread_count; thread_id++)
    {
        ...
        memory_pool_create(thread->thread_memory_pool, thread->thread_memory, sizeof(struct mediator_message));
    ...
    for (int32_t id = 0; id < threads.count; id++)
    {
        ...
        for (int32_t target = 0; target < target_count; target++)
        {
            for (int32_t message_id = 0; message_id < thread->whole_messages_count / target_count; message_id++)
            {
                ...
                message->owner = 0;
                message->method = 0;
                mediator_native_call_dart((struct mediator_native*)thread->test_mediator, targets[target], message);
            }
        }
        mediator_native_submit((struct mediator_native*)thread->test_mediator);
    }
    pthread_mutex_unlock((pthread_mutex_t*)threads.global_working_mutex);
}

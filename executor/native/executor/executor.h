...
DART_LEAF_FUNCTION int16_t executor_register_on_scheduler(struct executor_instance* executor, int64_t callback);
DART_LEAF_FUNCTION int16_t executor_unregister_from_scheduler(struct executor_instance* executor, bool stop);
DART_LEAF_FUNCTION int32_t executor_peek(struct executor_instance* executor);
DART_LEAF_FUNCTION int16_t executor_awake_begin(struct executor_instance* executor);
DART_LEAF_FUNCTION void executor_awake_complete(struct executor_instance* executor, uint32_t completions);
DART_LEAF_FUNCTION int16_t executor_call_native(struct executor_instance* executor, int32_t target_ring_fd, struct executor_task* message);
DART_LEAF_FUNCTION int16_t executor_callback_to_native(struct executor_instance* executor, struct executor_task* message);
...
{
    if (executor_need_submit(executor)) io_uring_submit(executor->ring);
}

DART_INLINE_LEAF_FUNCTION int16_t executor_call_dart(struct io_uring* ring, int32_t source_ring_fd, int32_t target_ring_fd, struct executor_task* message)
{
...
DART_INLINE_LEAF_FUNCTION int16_t executor_callback_to_dart(struct io_uring* ring, int32_t source_ring_fd, struct executor_task* message)
{
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    if (unlikely(sqe == NULL))
    {

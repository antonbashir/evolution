...
class ExecutorConsumerExecutor {
  ...
  @inline
  void call(Pointer<executor_message> message) => _callbacks[message.ref.method].call(message);
}

class ExecutorCallbackExecutor {
  final Pointer<executor_dart> _executor;
  final FutureOr<void> Function(Pointer<executor_message> notification) _executor;

  ExecutorCallbackExecutor(this._executor, this._executor);

  @inline
  void call(Pointer<executor_message> message) => Future.value(_executor(message)).then((_) => _respond(message));

  @inline
  void _respond(Pointer<executor_message> message) {

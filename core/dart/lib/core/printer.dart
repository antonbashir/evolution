...
class Printer {
  ...
  static void printOut(dynamic message) {
    if (!system().configuration.silent) context().coreModule().state.outPrinter(message?.toString() ?? empty);
  }

  static void printError(dynamic message) {
    if (!system().configuration.silent) context().coreModule().state.errorPrinter(message?.toString() ?? empty);
  }
}

void printEvent(Event event) {
  final configuration = system().configuration;

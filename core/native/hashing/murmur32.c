...
void murmur32_process(uint32_t *ph1, uint32_t *pcarry, const void *key, int len)
{
  uint32_t h1 = *ph1;
  uint32_t c = *pcarry;
...
uint32_t murmur32_result(uint32_t h, uint32_t carry, uint32_t total_length)
{
  uint32_t k1;
  int n = carry & 3;
  if(n) {
...
uint32_t murmur32(uint32_t seed, const void *key, int len)
{
  uint32_t h1=seed, carry=0;
  murmur32_process(&h1, &carry, key, len);
  return murmur32_result(h1, carry, len);
}

/*---------------------------------------------------------------------------*/

/* Provide an API suitable for smhasher */
void murmur32_hash_test(const void *key, int len, uint32_t seed, void *out)
{
  uint32_t h1=seed, carry=0;
  const uint8_t *ptr = (uint8_t*)key;
  ...
  while(ptr < end) {
    ...
    mid = mid<end?mid:end;
    murmur32_hash_process(&h1, &carry, ptr, mid-ptr);
    ptr = mid;
  }
#else
  murmur32_process(&h1, &carry, ptr, (int)(end-ptr));
#endif
  h1 = murmur32_result(h1, carry, len);
  *(uint32_t*)out = h1;
}

...
#define event_system_error(code, ...) event_error(strerror(code), event_field(EVENT_FIELD_CODE, code), ##__VA_ARGS__)
#define event_system_panic(code, ...) event_panic(strerror(code), event_field(EVENT_FIELD_CODE, code), ##__VA_ARGS__)

...
class StorageIndex {
  ...
  @inline
  Future<StorageIterator> iterator({StorageIteratorType iteratorType = StorageIteratorType.all}) {
    final (:Pointer<Uint8> value, :int size) = _tuples.emptyList;
  ...
  Future<StorageIterator> iteratorBy(
    Pointer<Uint8> key,
    int keySize, {
    StorageIteratorType iteratorType = StorageIteratorType.all,
  }) =>
  ...
  Future<StorageTuple> get(Pointer<Uint8> key, int keySize) {
    return _producer.indexGet(_descriptor, _factory.createIndex(_spaceId, _indexId, key, keySize)).then(_completeGet);
  }
